...
namespace Authentication.Configurations
{
    public class DatabaseIdentityServerInitializer
    {
        ...
        public void InitializeSeedRoles()
        {
            ...
            if (!_roleManager.RoleExistsAsync(Config.Admin).Result)
            {
                //cria o perfil Admin
                IdentityRole roleAdmin = new IdentityRole();
                roleAdmin.Name = Config.Admin;
                roleAdmin.NormalizedName = Config.Admin.ToUpper();
                _roleManager.CreateAsync(roleAdmin).Wait();
            }

            // se o perfil Client não existir então cria o perfil
            if (!_roleManager.RoleExistsAsync(Config.Client).Result)
            {
                //cria o perfil Client
                IdentityRole roleClient = new IdentityRole();
                roleClient.Name = Config.Client;
                roleClient.NormalizedName = Config.Client.ToUpper();
        ...
        public void InitializeSeedUsers()
        {
            ...
            if (_userManager.FindByEmailAsync("admin1@com.br").Result == null)
            {
                ...
                if (resultAdmin.Succeeded)
                {
                    ...
                    _userManager.AddToRoleAsync(admin, Config.Admin).Wait();
            ...
            if (_userManager.FindByEmailAsync("client1@com.br").Result == null)
            {
                ...
                {
                    _userManager.AddToRoleAsync(client, Config.Client).Wait();
        ...
        public void InitializeSeedDatabase()
        {
            ...
            if (_roleManager.FindByNameAsync(Config.Client).Result == null)
            {
                //cria o perfil Client
                IdentityRole roleClient = new IdentityRole();
                roleClient.Name = Config.Client;
